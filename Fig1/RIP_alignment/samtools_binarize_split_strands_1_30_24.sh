...
if [ $paired_end == "unpaired" ]; then
	...
	if [ $reversed_seq == "forward" ]; then
		...
		for file in $samfile_path/*.sam; do
			...
			sbatch -n 24 --wrap="samtools view -b -f 16 $file > strands/"$file_name"_-.bam"
		...
		for file in $samfile_path/*.sam; do
			...
			file_name=$(basename $file .sam)
                        ...
                        sbatch -n 24 --wrap="samtools view -b -f 16 $file > strands/"$file_name"_pos.bam"

                        # Next we map to the "forward" strand, excluding the 16 flag
						# Which is actually the reverse strand for our reversed data
                        sbatch -n 24 --wrap="samtools view -b -F 16 $file > strands/"$file_name"_neg.bam"
                done
	...
	if [ $reversed_seq == "forward" ]; then
		...
		for file in $samfile_path/*.sam; do
			...
			samtools merge -f strands/"$file_name"_-.bam strands/"$file_name"_-_83.bam strands/"$file_name"_-_163.bam"

			# Next we map to the "forward strand", forcing the 99 and 147 flags
			# 99 flag: READ1 mapped to forward, properly paired
			# 147 flag: READ2 mapped to reverse, properly paired.
			...
			samtools merge -f strands/"$file_name"_+.bam strands/"$file_name"_+_99.bam strands/"$file_name"_+_147.bam"
		done
		...
		for file in $samfile_path/*.sam; do
			...
			sbatch -n 24 --wrap="samtools view -b -f 83 $file > strands/"$file_name"_+_83.bam
			samtools view -b -f 163 $file > strands/"$file_name"_+_163.bam
			samtools merge -f strands/"$file_name"_pos.bam strands/"$file_name"_+_83.bam strands/"$file_name"_+_163.bam"

			# Next we map to the "forward" strand, forcing the 99 and 147 flag
			# Which is actually the reverse strand for our reversed data
			sbatch -n 24 --wrap="samtools view -b -f 99 $file > strands/"$file_name"_-_99.bam
			samtools view -b -f 147 $file > strands/"$file_name"_-_147.bam
			samtools merge -f strands/"$file_name"_neg.bam strands/"$file_name"_-_99.bam strands/"$file_name"_-_147.bam"
		done
